...
def main() -> None:
    ...
    copy_manifest_files(args.manifest_directory)
...
def replace_icon(m: dict) -> None:
    ...
    """
    icon_path = "bundle/icons/icon.png"
...
def copy_manifest_files(d: str) -> None:
    for filename in os.listdir(d):
        # the CSV file is a special case because we have to make changes to it
        # before we put it into the manifest directory. Everything else can get
        # copied over.
        if 'clusterserviceversion' in filename:
            continue
        src = os.path.join(d, filename)
        dest = shutil.copy(src, './manifests')
